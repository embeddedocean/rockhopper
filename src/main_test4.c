...
#define SPI_HEADER_NBYTES (8)
#define SPI_DMA_BUFFER_NBLOCKS (6)
...
//	}
	...
	uint8_t settings = AK5552_BITS_PER_SAMPLE;
	uint8_t year = 17;
	uint8_t month = 4;
	uint8_t day = 24;
	uint8_t hour = 0;
...
//				pmc_sleep(SAM_PM_SMODE_SLEEP_WFI);	
		...
		spi_header[7] = (uint8_t)(fs >> 24);
		spi_header[8] = (uint8_t)(fs >> 16);
		spi_header[9] = (uint8_t)(fs >> 8);
		spi_header[10] = (uint8_t)(fs >> 0);
		spi_header[11] = (uint8_t)(year);
		spi_header[12] = (uint8_t)(month);
		spi_header[13] = (uint8_t)(day);
		spi_header[14] = (uint8_t)(hour);
		spi_header[15] = (uint8_t)(minute);
		spi_header[16] = (uint8_t)(sec);
		spi_header[17] = (uint8_t)(usec >> 24);
		spi_header[18] = (uint8_t)(usec >> 16);
		spi_header[19] = (uint8_t)(usec >> 8);
		spi_header[20] = (uint8_t)(usec >> 0);
		spi_header[21] = 0xaa;
		spi_header[22] = 0xaa;

		uint8_t chksum = checksum(&spi_buffer[1], 22);
		spi_header[23] = chksum;

		// TESTING ONLY
		sec++;

		// write buffer to SPI
		int ret = spi_slave_write_wait((void *)spi_header, 24);

		//
		output_count = 0;			
		while( output_count < nblocks_per_upload ) {
	 	
			ret = sd_card_read_raw(&sd, spi_data, SPI_DMA_BUFFER_NBLOCKS, read_addr);
			...
			if( ret == SD_MMC_OK ) {
				...
				uint8_t flags = 0;
				if( (output_count + SPI_DMA_BUFFER_NBLOCKS) >= nblocks_per_upload) flags = 1;

				// update spi buffer header
				spi_header[0] = 0xaa;
				spi_header[2] = (uint8_t)(output_count >> 24);
				spi_header[3] = (uint8_t)(output_count >> 16);
				spi_header[4] = (uint8_t)(output_count >> 8);
				spi_header[5] = (uint8_t)(output_count >> 0);
				spi_header[6] = flags;
				spi_header[7] = 0xaa;

				uint8_t chksum = checksum(&spi_buffer[2], SPI_DMA_BUFFER_NBYTES-2);
				spi_header[1] = chksum;
