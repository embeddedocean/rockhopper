...
/* Pdc transfer buffer */
COMPILER_WORD_ALIGNED uint32_t ssc_adc_dma_buffer1[AK5552_MAX_DMA_BUFFER_NSAMPS];
COMPILER_WORD_ALIGNED uint32_t ssc_adc_dma_buffer2[AK5552_MAX_DMA_BUFFER_NSAMPS];
...
uint16_t ssc_adc_buffer_nsamps;
int ssc_adc_buffer_number = 1;
uint32_t ssc_adc_time = 0;
uint32_t ssc_adc_date = 0;

uint32_t ssc_adc_time_buffer1 = 0;
uint32_t ssc_adc_date_buffer1 = 0;
uint32_t ssc_adc_time_buffer2 = 0;
uint32_t ssc_adc_date_buffer2 = 0;
...
void SSC_Handler(void)
{
	...
	if ((status & SSC_SR_ENDRX) == SSC_SR_ENDRX) {
		...
		if(pdc_active_buffer_number == 1) {
			...
			ssc_adc_time_buffer2 = RTC->RTC_TIMR;
			ssc_adc_date_buffer2 = RTC->RTC_CALR;
			pdc_rx_init(ssc_pdc, NULL, &pdc_ssc_packet1);
			pdc_active_buffer_number = 2;
			ssc_adc_buffer = ssc_adc_dma_buffer1;
			ssc_adc_time = ssc_adc_time_buffer1;
			ssc_adc_date = ssc_adc_date_buffer1;
			...
			ssc_adc_time_buffer1 = RTC->RTC_TIMR;
			ssc_adc_date_buffer1 = RTC->RTC_CALR;
			pdc_rx_init(ssc_pdc, NULL, &pdc_ssc_packet2);
			pdc_active_buffer_number = 1;
			ssc_adc_buffer = ssc_adc_dma_buffer2;
			ssc_adc_time = ssc_adc_time_buffer2;
			ssc_adc_date = ssc_adc_date_buffer2;
...
uint16_t ak5552_dma_read(uint32_t **buffer)
{
	...
	*buffer = ssc_adc_buffer;
	...
	// update the buffer header info
	
	// indicate that the buffer has already been read
	ssc_adc_buffer = NULL;
	
	// return the buffer size in bytes
	return(ssc_adc_buffer_nsamps);
}

/**
 * \brief DMA driver configuration
 */
int ak5552_dma_init(uint16_t nsamps)
{
	...
	pdc_ssc_packet1.ul_size = (uint32_t)nsamps; //AK5552_DMA_BUFFER_NSAMPS;
	pdc_ssc_packet2.ul_addr = (uint32_t)ssc_adc_dma_buffer2;
	pdc_ssc_packet2.ul_size = (uint32_t)nsamps; //AK5552_DMA_BUFFER_NSAMPS;

	pdc_active_buffer_number = 1;
	ssc_adc_buffer = NULL;  // no buffer ready
	ssc_adc_buffer_nsamps = nsamps;
	
	/* Configure PDC for data receive */
	pdc_rx_init(ssc_pdc, &pdc_ssc_packet1, &pdc_ssc_packet2);

	printf("ak5552_dma_buffer_nsamps %lu \n\r", ssc_adc_buffer_nsamps);
